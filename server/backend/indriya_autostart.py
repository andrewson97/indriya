...
dcube_jobs_lock =  threading.Lock()
...
DCUBE_DELAY_START=120 # SECONDS TO GET THE JOB INTO RUNNING
DCUBE_DELAY_FINISHING=10 # SECONDS BEFORE DATA IS AVAILABLE AFTER FINISHED IS SET TO TRUE I
...
def is_correct_response(response):
	...
	if(response.status_code == 200):
		...
		return  json_response['evaluated'] == True
	else:
		return False

# def is_dcube_id_correct(dcube_id):
# 	# print("polling for dcube_id")
# 	return dcube_id != -1
...
# 	logger.info(result_id + " - " + dcube_jobs)
# 	logger.info(dcube_jobs[result_id])
# 	logger.info(dcube_jobs[result_id]['dcube_id'])
# 	dcube_id = -1
...
# 	logger.info(result_id + " " + dcube_id)
# 	return dcube_id
def save_dcube_results(result_id, dcube_id):
	working_dir = RESULT_DIRECTORY + result_id + "/dcube_"
	run_cmd("mkdir -p " + working_dir)
	run_cmd("wget  http://ocean.comp.nus.edu.sg/api/queue/evaluation/" 	+ str(dcube_id) + "?key=0oQ8FPEzRbXkjONLGEOHAq7OPFB8RvlymRASZYve7jED7HljaPgma0IceIcTJmnw -O " + working_dir + str(result_id) + "_" + str(dcube_id) + "_csv.zip")
	run_cmd("wget  http://ocean.comp.nus.edu.sg/api/queue/logs/" 		+ str(dcube_id) + "?key=0oQ8FPEzRbXkjONLGEOHAq7OPFB8RvlymRASZYve7jED7HljaPgma0IceIcTJmnw -O " + working_dir + str(result_id) + "_" + str(dcube_id) + "_logs.zip")


def poll_for_dcube_results(result_id):
	global dcube_jobs
	dcube_id = -1
	try:
		# for key in dcube_jobs.keys():
		# 	print(key,type(key), dcube_jobs[key])
		# 	logger.info(key + " " + str(dcube_jobs[key]))

		# print(result_id, dcube_jobs[result_id])
		dcube_id = str(dcube_jobs[result_id]['dcube_id'])
		logger.info("result_id " + result_id + " has dcube_id " + dcube_id)
	except:
		logger.warn("dcube_id " + str(dcube_id) + " not found in dcube_jobs")
		print(traceback.print_exc())

	logger.info("dcube_id," + dcube_id)
	if(dcube_id != -1):
		url='http://ocean.comp.nus.edu.sg/api/queue/' + dcube_id + '?key=0oQ8FPEzRbXkjONLGEOHAq7OPFB8RvlymRASZYve7jED7HljaPgma0IceIcTJmnw'
		...
		# sleep(DCUBE_DELAY_FINISHING)

		save_dcube_results(result_id, dcube_id)
	else:
		logger.info("No dcube related results for result_id " + result_id)


def compile_compress_data_for_job(json_data):
	global jobs_queue, running_jobs, dcube_jobs
	
	result_id = str(json_data['result_id'])
	

	logger.info("stopping job: " + result_id)
	...
	logger.info("polling for dcube related results (if any): " + result_id)

	try:
		if(dcube_jobs.get(result_id) != None):
			if(dcube_jobs[result_id].get('dcube_id') != None):
				poll_for_dcube_results(result_id)
	except:
		print(traceback.print_exc())

	logger.info("compiling and compressing data for result: " + result_id)
	# # wait_for_dcube_results(result_id)

	# # dcube_id = get_dcube_id(result_id)
...
# def wait_for_dcube_results(result_id):
# 	result=0
# 	dcube_id = -1
# 	while(True):
# 		if dcube_jobs.get(result_id) != None:
...
# 		if dcube_id != -1:
# 			command="curl  http://ocean.comp.nus.edu.sg/api/queue/" + dcube_id + "?key=0oQ8FPEzRbXkjONLGEOHAq7OPFB8RvlymRASZYve7jED7HljaPgma0IceIcTJmnw"
# 			# {"logs": false, "finished": true, "evaluated": true, "name": "12894", "failed": false, "running": false, "result": {"begin": "1550425247", "end": "1550426128"}, "schduled": "1550425246", "id": 57, "description": ""}
...
# 				results=1 if dcube_output["finished"]== "true" else 0
# 				logger.info("Results from dcube available for " + result_id)
# 				break
# 			except:
# 				pass # no results yet
# 				logger.info("No results YET from dcube for " + result_id)
# 				# print(traceback.print_exc())
# 		sleep(5)
...
def process_job(json_data):
	...
	# with FileLock("/tmp/" + result_id):
		...
		if(len(mote_list_burnt) > 0):# and job_is_not_cancelled_by_now):
			...
			###################################################################
			#### CHECK FOR DCUBE RELATED JOB
			###################################################################
			run_dcube_job_if_any(result_id)
			###################################################################
		else:
...
def run_dcube_job_if_any(result_id):
	logger.warn('job ' + result_id + ' : looking for dcube related job')
	if(dcube_jobs.get(result_id)!=None):
		...
		#####################################
		name=result_id
		time_to=dcube_jobs[result_id]['time_to']
		binary_file="/var/www/files/" + dcube_jobs[result_id]['binary_file']
		duration=str(int(time_to) - int(time()) - DCUBE_DELAY_START)

		logger.warn('dcube job ' + result_id + ' : ' + time_to + ", " + duration + ", " + binary_file)
		#####################################
		# call curl...
		# get the id
		#####################################
		command='curl -H "Content-Type: application/json" -X POST -d \'{"name":"' + name + '", "logs":"true", "duration":' + duration + ' ,"file": "\'$(cat ' + binary_file + '|base64 -w0)\'"}\' "http://ocean.comp.nus.edu.sg/api/queue/create_job?key=0oQ8FPEzRbXkjONLGEOHAq7OPFB8RvlymRASZYve7jED7HljaPgma0IceIcTJmnw"'
		logger.warn(command)
		...
		dcube_jobs_lock.acquire()
		dcube_jobs[result_id]['dcube_id'] = id
		# dcube_jobs.remove(result_id)
		dcube_jobs_lock.release()

	else:
		logger.warn('job ' + result_id + ' : has no dcube related job')
	# pass
...
def add_job_to_job_queue_and_scheduler(json_data):
	logger.info("add_job_to_job_queue_and_scheduler(json_data)")
	global first_run, jobs_queue, scheduler, hibernate
	try:
		result_id = json_data['result_id']
		if(hibernate):
			result = "0"
			logging.error("adding job during (from/to) hibernation process: " + result_id )
			return result
		...
		logger.info("end time:" + str(int(json_data['time']['to']) - GAP_BEFORE_STARTING_NEW_JOB + rnd_a_dp))
...
def hibernate():
	...
	dcube_jobs_lock.acquire()
	pickle.dump( dcube_jobs, open( "dcube_jobs.p", "wb" ) )
	dcube_jobs_lock.release()
...
def reload_jobs():
	...
	try:
		...
		dcube_jobs_lock.acquire()
		global dcube_jobs
		dcube_jobs = pickle.load( open( "dcube_jobs.p", "rb" ) )
		dcube_jobs_lock.release()
	except:
		print(traceback.print_exc())
		try:
...
@app.route("/dcube_jobs", methods=['GET','POST'])
def show_dcube_jobs():
	global dcube_jobs
	return str(dcube_jobs)

@app.route("/remove_dcube_job", methods=['GET','POST'])
def remove_dcube_job():
	global dcube_jobs
	result=0
	json_data=request.json
	result_id=json_data['result_id']
	...
	dcube_jobs_lock.acquire()
	if(dcube_jobs.get(result_id) != None):
		dcube_jobs.pop(result_id)
		result=1
	dcube_jobs_lock.release()

	response={}
	response['result_id']=result_id
	response['action']='remove_dcube_job'
	response['result']=result
	return str(response)
...
def new_job():
	...
	try:
		...
		if((int(json_data['time']['to']) - int(json_data['time']['from'])) > JOB_MIN_RUNNING_TIME and int(json_data['time']['to']) < now + JOB_MIN_RUNNING_TIME): # min running time
			...
			logger.info("Adding new job to queue")
...
@app.route("/new_dcube_job", methods=['GET','POST'])
def new_dcube_job():

	global dcube_jobs
	print("dcube_jobs",str(dcube_jobs))

	json_data_dcube = request.json
	logger.info("REQUEST: new DCUBE job " + json_data_dcube['result_id'] + " running until to " + json_data_dcube['time_to'])
	result_id=json_data_dcube['result_id']
	time_to=json_data_dcube['time_to']
	binary_file=json_data_dcube['binary_file']

	dcube_jobs_lock.acquire()
	dcube_jobs[result_id]={}
	dcube_jobs[result_id]['time_to']=time_to
	dcube_jobs[result_id]['binary_file']=binary_file
	dcube_jobs_lock.release()

	response={}
	response['result_id']=result_id
	response['action']='new_dcube_job'
	response['result']="1"
